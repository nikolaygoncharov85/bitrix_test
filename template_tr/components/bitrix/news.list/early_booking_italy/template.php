			...
			<div class="text_content">
				<div class="element_row">
					<?foreach($arItem["DISPLAY_PROPERTIES"]["hotel"]["LINK_ELEMENT_VALUE"] as $arHotel => $keyHotel) {?>
						...
						$x = CIBlockElement::GetProperty(32, $ID, Array(), Array("CODE"=>"early_booking_italy"));
						$y = $x->Fetch();
						$arC = CIBlockElement::GetProperty(32, $ID, Array(), Array("CODE"=>"CITY"));
						$arCity = $arC->Fetch();
						$arCityId = $arCity["VALUE"];
						$arCityN = CIBlockElement::GetByID($arCityId);
						if($arCName = $arCityN->GetNext())
						?>
						<div class="city_name"><?=$keyHotel["NAME"]?><?if(!empty($star)){?> <?=$star?>*<?}?></div>
						<span class="text"><?=$arCName["NAME"]?></span>
						<?if($y["VALUE"]["TEXT"]){?>
