...
if ($_REQUEST['arrFilter_pf']['COUNTRY'] > 0) {
    ...
    if ($arRow = $rsCountry->Fetch()) {
        ...
        $APPLICATION->AddChainItem($arRow['NAME'], '/tours-before-after-cruise/?arrFilter_pf[COUNTRY]=' . $arRow['ID'] . '&set_filter=Фильтр&set_filter=Y');
    ...
    if ($_REQUEST['arrFilter_pf']['CITY'] > 0) {
        ...
        if ($arRow = $rsCity->Fetch()) {
            ...
            */
            $title = "Место начала тура - " . $arRow['NAME'];
        }
