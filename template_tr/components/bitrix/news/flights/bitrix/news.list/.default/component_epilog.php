...
if ($_REQUEST['arrFilter_pf']['COUNTRY'] > 0) {
    ...
    if ($arRow = $rsCountry->Fetch()) {
        $name = (strlen($arRow['UF_TITLE3']) > 0) ? $arRow['UF_TITLE3'] : $arRow['NAME'];
        $title = $APPLICATION->GetTitle();
        $APPLICATION->AddChainItem($arRow['NAME'], '/flights/?arrFilter_pf[COUNTRY]=' . $arRow['ID'] . '&set_filter=Фильтр&set_filter=Y');

        $title = "Авиаперелет в " . $name;
    }
