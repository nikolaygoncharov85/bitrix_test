...
$GLOBALS['COUNTRY'] = $arResult['PROPERTIES']['COUNTRY']['VALUE'];
$APPLICATION->AddChainItem(
    strip_tags($arResult['PROPERTY_COUNTRY']),
    '/vip_countries/?arrFilter_pf[COUNTRY]=' . $GLOBALS['COUNTRY'] . '&set_filter=Фильтр&set_filter=Y'
);

if ($arResult['PROPERTIES']['CITY']['VALUE'] > 0 && $arResult['PROPERTY_CITY']) {
    $APPLICATION->AddChainItem(
        strip_tags($arResult['PROPERTY_CITY']),
        '/vip_countries/?arrFilter_pf[COUNTRY]=' . $GLOBALS['COUNTRY'] . '&arrFilter_pf[CITY]=' . $arResult['PROPERTIES']['CITY']['VALUE'] . '&set_filter=Фильтр&set_filter=Y'
    );
}
