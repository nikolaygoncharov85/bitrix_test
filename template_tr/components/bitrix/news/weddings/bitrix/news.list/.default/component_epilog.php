...
if ($_REQUEST['arrFilter_pf']['COUNTRY'] > 0) {
    ...
    if ($arRow = $rsCountry->Fetch()) {
        ...
        $APPLICATION->AddChainItem($arRow['NAME'], '/weddings/?arrFilter_pf[COUNTRY]=' . $arRow['ID'] . '&set_filter=Фильтр&set_filter=Y');

        $title = "Свадебные туры " . $name;
    }
