...
if ($_REQUEST['resort'] == 'Y') {
    if (strlen($_POST['countryId']) > 0) {
        ...
        if ($sectionId > 0) {
            ...
            if (count($arr['REFERENCE']) == 0) {
                ...
                $res = $element->GetList(
                    array('ID' => 'DESC'),
                    array('IBLOCK_ID' => 27, 'SECTION_ID' => $sectionId, 'ACTIVE' => 'Y', '!PROPERTY_HIDE_IN_HOTELS' => 'Y'),
                    false,
    ...
    $result = array(
        ...
        'city' => SelectBoxFromArray("arrFilter_pf[CITY]", $arCity, $_REQUEST['arrFilter_pf[CITY]'], "---", " class=\"custom-select\" data-placeholder=\"Место начала тура\"", false, "")
    );
...
if ($_REQUEST['city'] == 'Y') {
    ...
    echo SelectBoxFromArray("arrFilter_pf[CITY]", $arr, $_REQUEST['arrFilter_pf[CITY]'], "---", " class=\"custom-select\" data-placeholder=\"Место начала тура\"", false, "");
}