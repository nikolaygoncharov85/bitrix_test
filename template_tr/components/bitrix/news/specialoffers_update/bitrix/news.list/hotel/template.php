...
if(!empty($_REQUEST["arrFilter_pf"]["COUNTRY"])){
    $url_country = '&arrFilter_pf%5BCOUNTRY%5D='.$_REQUEST["arrFilter_pf"]["COUNTRY"];
}
$url_type = $arrFilter_hotel["PROPERTY_SPO_BLOCK"][0]
?>
<? if (!empty($arResult["ITEMS"])){?>
    <div class="row-1 row-inner white-bg mb-col-dist">
        <div class="col relative">
            <h2 class="mt-col-dist hotels_spo_h2"><?=$arParams["BLOCK_HEAD"]?></h2>
            <a class="hotels_spo_all absolute" href="/specialoffers/?&arrFilter_pf%5BTYPE%5D=<?=$url_type?><?=$url_country?>&set_filter=Фильтр&set_filter=Y">смотреть все</a>
